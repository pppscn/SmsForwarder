...
import cn.ppps.forwarder.databinding.FragmentSendersWpushBinding
...
class WpushFragment : BaseFragment<FragmentSendersWpushBinding?>(), View.OnClickListener {

    private val TAG: String = WpushFragment::class.java.simpleName
    private var titleBar: TitleBar? = null
    private val viewModel by viewModels<SenderViewModel> { BaseViewModelFactory(context) }
    private var mCountDownHelper: CountDownButtonHelper? = null

    private val allowedChannels = setOf(
        "wechat", "app", "sms", "mail", "webhook",
        "dingtalk", "feishu", "wechat_work", "clawbot", "qqbot"
    )
    ...
    override fun viewBindingInflate(
        ...
        container: ViewGroup,
    ): FragmentSendersWpushBinding {
        return FragmentSendersWpushBinding.inflate(inflater, container, false)
    }

    override fun initTitle(): TitleBar? {
        titleBar = super.initTitle()!!.setImmersive(false).setTitle(R.string.wpush)
        return titleBar
    }
    ...
    override fun initViews() {
        ...
        Core.sender.get(senderId).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(object : SingleObserver<Sender> {
            ...
            override fun onSuccess(sender: Sender) {
                ...
                if (settingVo != null) {
                    binding!!.etApikey.setText(settingVo.apiKey)
                    binding!!.etChannel.setText(if (TextUtils.isEmpty(settingVo.channel)) "wechat" else settingVo.channel)
                    binding!!.etTopicCode.setText(settingVo.topicCode)
                ...
                R.id.btn_test -> {
                    ...
                    Thread {
                        try {
                            ...
                            WpushUtils.sendMsg(settingVo, msgInfo)
                        } catch (e: Exception) {
                            e.printStackTrace()
    ...
    private fun checkSetting(): WpushSetting {
        val apiKey = binding!!.etApikey.text.toString().trim()
        if (TextUtils.isEmpty(apiKey)) {
            throw Exception(getString(R.string.invalid_wpush_apikey))
        }
        if (!apiKey.matches(Regex("(?i)^WPUSH[a-z0-9]+$"))) {
            throw Exception(getString(R.string.invalid_wpush_apikey))
        }

        val channel = binding!!.etChannel.text.toString().trim().ifEmpty { "wechat" }.lowercase()
        if (channel !in allowedChannels) {
            throw Exception(getString(R.string.invalid_wpush_channel))
        }

        val topicCode = binding!!.etTopicCode.text.toString().trim()
        val titleTemplate = binding!!.etTitleTemplate.text.toString().trim()

        return WpushSetting(apiKey, channel, topicCode, titleTemplate)
    }
