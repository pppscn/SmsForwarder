...
import cn.ppps.forwarder.databinding.FragmentSendersWxpusherBinding
...
class WxpusherFragment : BaseFragment<FragmentSendersWxpusherBinding?>(), View.OnClickListener {

    private val TAG: String = WxpusherFragment::class.java.simpleName
    ...
    override fun viewBindingInflate(
        ...
        container: ViewGroup,
    ): FragmentSendersWxpusherBinding {
        return FragmentSendersWxpusherBinding.inflate(inflater, container, false)
    }

    override fun initTitle(): TitleBar? {
        titleBar = super.initTitle()!!.setImmersive(false).setTitle(R.string.wxpusher)
        return titleBar
    }
    ...
    override fun initViews() {
        ...
        //创建标签按钮
        CommonUtils.createTagButtons(requireContext(), binding!!.glSummaryTemplate, binding!!.etSummaryTemplate)
        ...
        Core.sender.get(senderId).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(object : SingleObserver<Sender> {
            ...
            override fun onSuccess(sender: Sender) {
                ...
                if (settingVo != null) {
                    binding!!.rgPushType.check(settingVo.getPushTypeCheckId())
                    binding!!.etAppToken.setText(settingVo.appToken)
                    binding!!.etUids.setText(settingVo.uids)
                    binding!!.etTopicIds.setText(settingVo.topicIds)
                    binding!!.etSpt.setText(settingVo.spt)
                    binding!!.rgContentType.check(settingVo.getContentTypeCheckId())
                    binding!!.etSummaryTemplate.setText(settingVo.summaryTemplate)
                    binding!!.etRegexReplace.setText(settingVo.regexReplace)
                }
                togglePushTypeLayouts()
            }
        })
    }
    ...
    override fun initListeners() {
        ...
        binding!!.rgPushType.setOnCheckedChangeListener { _: RadioGroup?, _: Int -> togglePushTypeLayouts() }
        LiveEventBus.get(KEY_SENDER_TEST, String::class.java).observe(this) { mCountDownHelper?.finish() }
    }

    //切换标准推送与SPT极简推送的显示
    private fun togglePushTypeLayouts() {
        if (binding!!.rgPushType.checkedRadioButtonId == R.id.rb_simple_push) {
            binding!!.layoutStandardPush.visibility = View.GONE
            binding!!.layoutSimplePush.visibility = View.VISIBLE
        } else {
            binding!!.layoutStandardPush.visibility = View.VISIBLE
            binding!!.layoutSimplePush.visibility = View.GONE
        }
    }
                ...
                R.id.btn_test -> {
                    ...
                    Thread {
                        try {
                            ...
                            WxpusherUtils.sendMsg(settingVo, msgInfo)
                        } catch (e: Exception) {
                            e.printStackTrace()
    ...
    private fun checkSetting(): WxpusherSetting {
        val appToken = binding!!.etAppToken.text.toString().trim()
        val uids = binding!!.etUids.text.toString().trim()
        val topicIds = binding!!.etTopicIds.text.toString().trim()
        val spt = binding!!.etSpt.text.toString().trim()
        val summary = binding!!.etSummaryTemplate.text.toString().trim()
        val regexReplace = binding!!.etRegexReplace.text.toString().trim()

        if (binding!!.rgPushType.checkedRadioButtonId == R.id.rb_simple_push) {
            //SPT极简推送
            if (TextUtils.isEmpty(spt)) {
                throw Exception(getString(R.string.invalid_wxpusher_spt))
            }
        } else {
            //标准推送
            if (TextUtils.isEmpty(appToken)) {
                throw Exception(getString(R.string.invalid_wxpusher_app_token))
            }
            if (TextUtils.isEmpty(uids) && TextUtils.isEmpty(topicIds)) {
                throw Exception(getString(R.string.invalid_wxpusher_uids))
            }
        }

        val contentType = when (binding!!.rgContentType.checkedRadioButtonId) {
            R.id.rb_content_html -> 2
            R.id.rb_content_markdown -> 3
            else -> 1
        }

        return WxpusherSetting(appToken, uids, topicIds, spt, contentType, summary, regexReplace)
    }
